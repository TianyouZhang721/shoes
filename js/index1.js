$(()=>{
	...
	$.ajax({
		...
		`;
		$Inimgs.html(html).css("width",LIWIDTH*(data.length+1));
		...
		function move(dir,callback){
			moved+=dir;
			if(moved<data.length){
				$listCon.children(":eq("+moved+")").children().children().addClass('active');
				$listCon.children(":eq("+moved+")").siblings().children().children().removeClass('active');
			}else{
				$listCon.children(":eq(0)").children().children().addClass('active')
				$listCon.children(":eq(0)").siblings().children().children().removeClass('active');
			}
